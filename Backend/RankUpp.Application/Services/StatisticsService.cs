...
namespace RankUpp.Application.Services
{
    public class StatisticsService : IStatisticsService
    {
        ...
        public async Task<UserStatistics> CalculateUserStatistics(int userId)
        {
            ...
            userStatistics.LastWeekPoints = await _userRepository.GetPointsChangeByTimeAsync(userId, DateTime.UtcNow.AddDays(-7));

            userStatistics.LastWeekActivityCount = await _userRepository.GetActivityTimeAsync(userId, DateTime.UtcNow.AddDays(-7));
